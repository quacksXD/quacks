    ...
    */
    ...
    var basicBot = {
        ...
        settings: {
            ...
            commandLiteral: '!',
            blacklists: {
                NSFW: 'https://rawgit.com/Vnysz/MafiaBot/master/blacklists/NSFWlist.json',
                OP: 'https://rawgit.com/Vnysz/MafiaBot/master/blacklists/OPlist.json',
                BANNED: 'https://rawgit.com/Vnysz/MafiaBot/master/blacklists/BANNEDlist.json'
            }
        },
        room: {
            name: null,
        ...
        eventVoteupdate: function(obj) {
            ...
            var length = API.getUsers().length;
            var dj = API.getDJ();
            ...
            autofavCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (basicBot.settings.autofav) {
                            basicBot.settings.autofav = !basicBot.settings.autofav;
                            return API.sendChat(subChat(basicBot.chat.toggleoff, {name: chat.un, 'function': basicBot.chat.autofav}));
                        }
                        else {
                            basicBot.settings.autofav = !basicBot.settings.autofav;
                            return API.sendChat(subChat(basicBot.chat.toggleon, {name: chat.un, 'function': basicBot.chat.autofav}));
                        }
            ...
            */
            ...
            discordCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void(0);
                        ...
                        var link = 'https://discord.gg/c66sn3n';
            ...
            favCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        return API.sendChat(subChat(basicBot.chat.fav));
                    }
                }
            },
            ...
            emotesCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void(0);
                        ...
                        if (typeof basicBot.settings.emotesLink === 'string')
                            API.sendChat(subChat(basicBot.chat.emotes, {
                                link: basicBot.settings.emotesLink
                            }));
                    }
                }
            },       
            ...
            kissCommand: {
                ...
                getKisses: function (chat) {
                    var sho = Math.floor(Math.random() * basicBot.chat.kisses.length);
                    return basicBot.chat.kisses[sho];
                },
                functionality: function (chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(basicBot.chat.eatkiss);
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                            if (name === "djs") {
                                return API.sendChat(subChat(basicBot.chat.multikiss, {namefrom: chat.un, weed: this.getKisses()}));
                            }
             ...
             cantadaCommand: {
                command: ['cantada', 'ct', 'conquistar'],
                rank: 'user',
                type: 'startsWith',
                getCantada: function(chat) {
                    var c = Math.floor(Math.random() * basicBot.chat.cantadass.length);
                    return basicBot.chat.cantadass[c];
                },
                functionality: function(chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void(0);
                    if (!basicBot.commands.executable(this.rank, chat)) return void(0);
            ...
            xavecoCommand: {
                ...
                functionality: function(chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void(0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(basicBot.chat.eatxaveco);
                            return false;
                        } else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                        ...
                        for (var i = 0; i < len; ++i) {
                            if (users[i].username == name) {
                                ...
                                if (rawlang == "en"){
                                    ...
                                    var language = "Búlgaro";
                                } else if (rawlang == "cs"){
                                    var language = "Tcheco";
                                } else if (rawlang == "fi"){
                                    var language = "Finlandês"
                                } else if (rawlang == "fr"){
                                    var language = "Francês"
                                } else if (rawlang == "pt"){
                                    var language = "Português"
                                } else if (rawlang == "zh"){
                                    var language = "Chinês"
                                } else if (rawlang == "sk"){
                                    var language = "Eslovaco"
                                } else if (rawlang == "nl"){
                                    var language = "Holandês"
                                } else if (rawlang == "ms"){
                                    var language = "Malaio"
                                } else if (rawlang == "pl"){
                                    var language = "Polonês" 
                                } else if (rawlang == "bs"){
                                    var language = "Bosniano" 
                                } else if (rawlang == "da"){
                                    var language = "Dinamarquês"   
                                } else if (rawlang == "de"){
                                    var language = "Alemão" 
                                } else if (rawlang == "es"){
                                    var language = "Espanhol"
                                } else if (rawlang == "hu"){
                                    var language = "Hungarian"
                                } else if (rawlang == "it"){
                                    var language = "Italiano"
                                } else if (rawlang == "ja"){
                                    var language = "Japonês"
                                } else if (rawlang == "tr"){
                                    var language = "Turco"
                                } else if (rawlang == "ko"){
                                    var language = "Coreano"
                                } else if (rawlang == "lt"){
                                    var language = "Lituano"
                                } else if (rawlang == "lv"){
                                    var language = "Letão"
                                } else if (rawlang == "nb"){
